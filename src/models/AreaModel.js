...
class AreaModel {
  ...
  constructor(props) {
    let entries = props.entries || props.series

    if(!_.isArray(entries)) { throw new TypeError("entries/series must be an array") }

    this.entries = decorateTimeSeries(entries)
  ...
  get deltaPercentageSeries() {
    ...
    let x = this.entries.map((curr, i, a) => {
      let prev = a[i-1]

      let newDeltas = deltaMap.reduce((h, [baseKey, deltaKey]) => {
        let val = (prev && _.isFinite(prev[baseKey]) && prev[baseKey] > 0) ? 100*(curr[baseKey]/prev[baseKey]-1) : null

        return Object.assign(h, {[deltaKey]: val})
      }, {})
